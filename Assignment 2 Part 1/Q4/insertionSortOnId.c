/*
4. Declare an array of 30 student structures where each structure contains the fields,
 name, idno and marks. Write a C Program that sorts the above array of students
 structures on the idno field using insertion sort.
*/
...
int main()
{
    ...
    //Insertion Sort Algorithm to sort students by idno
    for (i = 1; i < n; i++)
    {
        temp=s[i];//Store the current student in temp
        //Shift students with greater id_no to the right
        for (j = i-1;j>=0 && temp.id_no<s[j].id_no; j--)//Compare id_no
        {
    ...
    printf("\n--- Students Sorted by Idno (Insertion Sort) ---\n");
